...
'''
dummy-status.py that actus as the regular status page
statuses are hard-coded to demo the ideal page 
Used only for demostrations and testing prototypes
'''


app = flask.Flask(__name__, static_folder='static', template_folder='templates')

def get_status_dict_good():
    timestamp = datetime.now()
    get_status.update_status()
    overall_status = 'healthy_container' 
    status_history_list = get_status.get_status_history(30)
    ...
    vm1_status_circ = 'status-circle-healthy-container'
    vm2_status_circ = 'status-circle-healthy-container'
    http_status_circ = 'status-circle-healthy-container' 
    login_status_circ = 'status-circle-healthy-container' 
    register_status_circ = 'status-circle-healthy-container'
    board_status_circ = 'status-circle-healthy-container'
    vm1_sd_circ = 'status-circle-healthy-container'
    vm2_sd_circ = 'status-circle-healthy-container' 
    vm1_dn_circ = 'status-circle-healthy-container'
    vm2_dn_circ = 'status-circle-healthy-container' 
...
def get_status_dict_bad():
    timestamp = datetime.now()
    get_status.update_status()
    overall_status = 'issue_container'
    status_history_list = get_status.get_status_history(30)
    status_history_list_length = len(status_history_list)
    status_history_percentage = round((sum(status_tuple[1] for status_tuple in status_history_list) / status_history_list_length) * 100, 2) if status_history_list_length > 0 else 0
    vm1_status_circ = 'status-circle-healthy-container' 
    vm2_status_circ = 'status-circle-healthy-container' 
    http_status_circ = 'status-circle-issue-container'
    login_status_circ = 'status-circle-issue-container'
    register_status_circ = 'status-circle-issue-container'
    board_status_circ = 'status-circle-issue-container'
    vm1_sd_circ = 'status-circle-healthy-container' 
    vm2_sd_circ = 'status-circle-healthy-container' 
    vm1_dn_circ = 'status-circle-issue-container'
    vm2_dn_circ = 'status-circle-issue-container'
